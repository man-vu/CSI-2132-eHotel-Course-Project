...
const UpdateModal: FC<UpdateModalProps> = ({ isOpen, onClose, onSave, entity, initialData }) => {
  const [formData, setFormData] = useState(initialData);
  const [error, setError] = useState<string | null>(null);
  const [invalidFields, setInvalidFields] = useState<string[]>([]);

  console.log("UpdateModal initialData:", initialData);
  ...
  const handleSubmit = () => {
    ...
    //   (key) => formData[key].required && !formData[key].value && formData[key].type !== 'hidden'
    // );
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-6 rounded shadow-lg w-96">
        <h3 className="text-lg font-semibold mb-4">Update {entity.slice(0, -1)}</h3>
